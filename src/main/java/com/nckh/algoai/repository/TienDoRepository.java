...
public interface TienDoRepository extends JpaRepository<TienDoEntity, Integer> {
    ...
    @Query("SELECT td FROM TienDoEntity td WHERE td.idNguoiDung = :idNguoiDung AND td.baiHoc.idKhoaHoc = :idKhoaHoc")
    List<TienDoEntity> findByIdNguoiDungAndIdKhoaHoc(@Param("idNguoiDung") Integer idNguoiDung,
